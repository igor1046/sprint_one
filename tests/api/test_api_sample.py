...
def test_put_req(create_id):
    payload = {
        "id": create_id,
        "category": {"id": random.randint(100, 999), "name": "NEWTestCategory"},
        "name": "NEWTestName",
        "photoUrls": ["string"],
        "tags": [{"id": random.randint(100, 999), "name": "NEWTestTag"}],
        "status": "available",
    }

    response = requests.put("https://petstore.swagger.io/v2/pet", json=payload).json()
    print(response)
